...
class CommandController : NSWindowController {
  ...
  /* let zz = [buildXIBAbout, // buildXIBMainMenu,
   buildXIBQSCatalog, buildXIBQSTaskView, buildXIBQSWebSource, buildXIBQSTaskViewer, // 1
   buildXIBQuitConfirm, buildXIBQSPreferences, buildXIBCommandBuilder, buildXIBQSSearchPrefPane, buildXIBQSSetupAssistant, // 5
   buildXIBQSSimpleNotifier, buildXIBQSActionsPrefPane, buildXIBQSPlugInsPrefPane, buildXIBQSPreferences, buildXIBQSCatalog, // 10
   buildXIBQSUserDefinedProxySource, buildXIBQSFileSystemObjectSource, buildXIBQSDefaultsObjectSource, // 15
   buildXIBQSProcessObjectSource, buildXIBQSApplicationPrefPane, buildXIBQSAppearancePrefPane, buildXIBQSFileConflictPanel, // 18
   buildXIBQSAdvancedPrefPane, buildXIBQSTriggersPrefPane, buildXIBQSMainMenuPrefPane, ] // 22
   */
  ...
  public init() {
    ...
    let w = CommandWindow()
    w.setFrameUsingName("CommandWindow")
    w.setFrameAutosaveName("CommandWindow")
    // w.center()
  ...
  func handleHotKey() {
    ...
    let z = self.window!.frame
      let y = z.insetBy(dx: 0, dy: z.height / 2)
      self.window?.setFrame(y, display:  true, animate: true)
     // self.showWindow(nil)
    ...
    NSAnimationContext.runAnimationGroup( { context in
      ...
      print(z)
      self.window?.animator().setFrame( z, display: true, animate: true )
...
class InterfaceController : NSWindowController {
  var dSelector : QSSearchObjectView? // direct object
  var aSelector : QSSearchObjectView? // action
  var iSelector : QSSearchObjectView? // indirect object
  
  var commandView : NSView?
  // @IBOutlet var menuButton : QSMenuButton?
  
  var hideTimer : Timer?
  var actionsUpdateTimer : Timer?
  ...
  var hidingWindow : Bool = false
  var preview : Bool = false
  
  // var savedKeyboard : TISInputSourceRef
  ...
  /*
   ...
   self->savedKeyboard = TISCopyCurrentKeyboardLayoutInputSource();
   ...
   #define kQSBundleID @"net.r0ml.ClickSliver"
   ...
   [QSHistoryController.sharedInstance addCommand:[self currentCommand]];
...
class TransparentView : NSView {
  ...
  public override func draw(_ frame : CGRect) {
    let path = NSBezierPath(roundedRect: frame, xRadius:6.0, yRadius:6.0)
    NSColor.gray.set()
    path.fill()
  }
}
...
class CommandWindow : NSPanel {
  public init() {
    super.init(contentRect: CGRect(x: 100, y: 100, width: 600, height: 200), styleMask: [.borderless, .hudWindow], backing: .buffered, defer: false)
    self.isOpaque = false
    self.alphaValue = 0.75
    self.backgroundColor = NSColor.clear
    self.contentView = TransparentView()
    let v = NSTextField()
    v.addInto(self.contentView!,with: [.width, .leading, .centerY])
...
class QSObjectView : NSControl {
  ...
  /*
   ...
   NSString *action = [[self objectValue] actionForDragOperation:operation with:draggedObject];
   ...
   NSString *action = [ [self objectValue] actionForDragOperation:lastDragMask with:draggedObject];
...
class QSSearchObjectView : QSObjectView, NSTextInputClient, NSTextViewDelegate {
  ...
  /*
   ...
   NSString *action = [[self objectValue] actionForDragOperation:NSDragOperationEvery with:newSelection];
   //NSLog(@"action %@", action);
   ...
   - (void)resetString {
   QSGCDMainSync(^{
   [self->resultController.searchStringField setTextColor:[[resultController.searchStringField textColor] colorWithAlphaComponent:0.5]];
   [self->resultController.searchStringField display];
   });
   }
   ...
   NSNumber *typeBehavior = [[[QSRegistry tableNamed:@"QSTypeDefinitions"] objectForKey:[newSelectedObject primaryType]] objectForKey:kQSSmartSpace];
   if (typeBehavior) {
   ...
   if (lastObjectValue) {
   [QSHistoryController.sharedInstance addObject:lastObjectValue];
   }
...
class PrimerInterfaceController : InterfaceController {
  //   NSRect standardRect;
  @IBOutlet var executeButton : NSButton?
  
  @IBOutlet var dSearchText : NSTextField?
  @IBOutlet var aSearchText : NSTextField?
  @IBOutlet var iSearchText : NSTextField?
  
  @IBOutlet var dSearchCount : NSTextField?
  @IBOutlet var aSearchCount : NSTextField?
  @IBOutlet var iSearchCount : NSTextField?
  
  @IBOutlet var dSearchResultDisclosure : NSButton?
  @IBOutlet var aSearchResultDisclosure : NSButton?
  @IBOutlet var iSearchResultDisclosure : NSButton?
  
  @IBOutlet var indirectView : NSView?
